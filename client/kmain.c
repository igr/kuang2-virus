...
// ver 0.20 (10-may-1999): puno sređivanja
...
OPENFILENAME ofn = {
    sizeof(OPENFILENAME),       // veličina ove strukture
...
/*
  ...
  + čisti string za ime fajla - sklanja veličinu fajla */
...
/*
    Uploadloop
    ----------
  + Obrada poruka za pomoćni dijalog. */
...
/*
    Runloop
    --------
  + Obrada poruka za pomo†ni dijalog.
...
/*
    OnTreeView_Rbutton
    ------------------
  + otpuštanje desnog dugmeta miša - radi PopUp meni u treeView kontroli. */
...
void OnTreeView_Rbutton(HWND hwnd)
{
    ...
    if (!connected) return;                 // ako nismo konektovani, izađi

    GetWindowRect(hTV, &rc);                // uzmi koordinate treeView-a
    GetCursorPos(&pt);                      // uzmi koordinate kurzora

    if (!PtInRect(&rc, pt)) return;         // ako nije u prozoru, izađi

    hittest.pt.x=pt.x-rc.left;              // ok, klik je u treeview prozoru
    hittest.pt.y=pt.y-rc.top;               // pređi na client koordinate klika
    ...
    DestroyMenu(hmenu);                     // na kraju obriši meni
    return;
}
...
/*
    preuzmi
    -------
  + pomoćna funkcja, sređuje string kada ga meni izabere */
...
/*
    UnseeIt
    -------
  + pomoćna funkcja, izabrani folder namešta na unseen.
...
/*
    SetDlg
    ------
  + pomoćna funkcja, kada se menja veličina dijaloga sređuje pomerljive
    komponente. */
...
LRESULT CALLBACK Kmsgloop (HWND hd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    switch (msg) {
        ...
        case WM_INITDIALOG: // inicijalizacija dialoga PRE nego što se dijalog otvori
            hDlg=hd;
            SendDlgItemMessage(hDlg, ID_EDIT, EM_SETLIMITTEXT, 15, 0);      // ograniči za svaki slučaj
            hTV=GetDlgItem(hDlg, ID_TV);    // uzmi handle od Tree-View-a
            TreeView_SetIndent(hTV, 8);     // određuje ident od 8 pixela
            tvInsert.hInsertAfter = TVI_LAST;
            tvItem.mask = TVIF_TEXT | TVIF_CHILDREN | TVIF_PARAM | TVIF_IMAGE | TVIF_SELECTEDIMAGE;
            tvItem.lParam=0;
            hIcon1=LoadIcon(hThisInst, "DOC");              // učitaj ikone
            ...
            TreeView_SetImageList(hTV, hIlist, TVSIL_NORMAL);// poveži image list sa tree view
            ikona=LoadIcon(hThisInst, "AAA");                           // učitaj ikonu programa
            ...
            for (i=0; i<5; i++) eporuka[i+1]=coded[i];      // dodaj reč 'coded'
            SetDlgItemText(hDlg, ID_STATUS, &eporuka[1]);   // ispiši poruku u statusu
        ...
        case WM_GETMINMAXINFO: {    // zahteva se informacija o min/max dimenzijama dijaloga
            ...
            lpMMI->ptMinTrackSize.x=263;        // minimalna širina
            lpMMI->ptMinTrackSize.y=220;        // minimalna visina
            GetWindowRect(hd, &r);
            lpMMI->ptMaxSize.x=r.right-r.left;  // nepromenljiva širina
            lpMMI->ptMaxPosition.x=r.left;      // nepromenljiva x koordinate
            return 0;       // vra†a se 0 ako je ova poruka obra”ena
            }
        ...
        case WM_SIZE:       // pomeranje je završeno ili je urađen min/max/restore
            GetWindowRect(hd, &r);
            SetDlg(r);
            InvalidateRect(hDlg, NULL, FALSE);  // osveži prozor
            return TRUE;
        ...
        case WM_SYSCOMMAND:
            ...
            if (wParam==SC_MINIMIZE) {      // pre minimizacije
                minimised=TRUE;             // zapamti da †e biti minimizovan
                max_pa_min=IsZoomed(hDlg);  // zapamti da li je pre toga bio maximizovan
                break;
            }
            if (wParam==SC_RESTORE) {   // pre restore!
                OpenIcon(hDlg);
                if (max_pa_min) {
                    max_pa_min=minimised=FALSE;     // nije više minimizovan
        ...
        case UM_ASYNCEND:                       // obaveštavanje glavnog programa
            if ((wParam==173)&&(lParam==173)) { // da je asinhrona komanda završena
