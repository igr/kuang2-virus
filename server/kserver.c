...
/* HISTORY */
// ver 0.15 (06-may-1999): konačan oblik
...
/*
	StartServer
	-----------
  + Početak rada servera. Kreira se socket, postavlja se asinhrono
	obaveštavanje i uključuje se listen mod */
...
int StartServer(void)
{
	...
	// postavi asinhrono obaveštavanje
...
void OnAccept(SOCKET socket)
{
	...
	nlen=NewRequest();
	if (nlen==NOMOREREQUEST) {			// ima previše konektovanih klijenata
...
/*
	OnQuitclient
	------------
  + dobijena je poruka za završavanje klijenta */
...
void OnQuitclient(SOCKET socket)
{
	...
	// obriši zauzeto polje u Request listi
	request[i].socket=-1;
...
/*
	ServerThread
	------------
  + Svaka konekcija koja se zakači na server koristi ovaj thread. */
...
#define		downloaded		tosend
	...
	fajlsize=0;
	// ovo je kulturan server - šalje pozdravnu poruku pri konekciji
	...
	while (1) {
		...
		if (!bytes_recieved) k2_msg->command=K2_QUIT;			// ako je klijent otišao
		if ((bytes_recieved==SOCKET_ERROR)) {					// ako je nastupila greška
			if (WSAGetLastError()!=WSAEWOULDBLOCK) k2_msg->command=K2_QUIT; // i to fatalna
				else continue;									// ne fatalna greška - wsaewouldblock
		}
		...
		switch (k2_msg->command) {
			...
			case K2_DOWNLOAD_FILE:
				...
				// faza #2 - započinje burst prenos fajla
				if (k2_msg->param==2) {
					while (fajlsize) {
						tosend=BUFFER_SIZE;
						...
						bytes_sent=send(msgsock, buffer, tosend, 0);	// pošalji 1KB fajla
				...
				if ((fajlsize==0xFFFFFFFF) || (fajl==INVALID_HANDLE_VALUE)) {
					k2_msg->command=K2_ERROR;		// neka greška pri radu sa fajlom
					send(msgsock, buffer, 4, 0);
					break;
				}
				k2_msg->command=K2_DONE;			// fajl spreman za download
				k2_msg->param=fajlsize;				// veličina fajla
			...
			case K2_FOLDER_INFO: {
				...
				hSearch=FindFirstFile(k2_msg->bdata, &FileData);	// traži...
				if ((fajl==INVALID_HANDLE_VALUE) || (hSearch==INVALID_HANDLE_VALUE)) {	// ako ne nađe prvi fajl ili fajl nije otvoren
				...
				while (!fFinished) {	// vrti za sve fajlove, 'buffer' je slobodan
					...
					if (FileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
						...
						temp=FileData.nFileSizeLow >> 10;		// veličina fajla u KB
					...
					// upiši string u fajl, zajedno sa NULL terminatorom na kraju
					WriteFile(fajl, buffer, strlengthF(buffer)+1, &temp, NULL);

					// uzima sledeći fajl
					if (!FindNextFile(hSearch, &FileData)) {
						if (GetLastError()==ERROR_NO_MORE_FILES) {	// došli smo na kraj - nema više fajlova
							k2_msg->command=K2_DONE;				// pošalji oznaku za kraj
							strcopyF(k2_msg->bdata, filelist);		// i ime fajla za download
						} else k2_msg->command=K2_ERROR;			// nastupila je greška
			...
			case K2_UPLOAD_FILE:
				...
				if (fajl==INVALID_HANDLE_VALUE) {
					k2_msg->command=K2_ERROR;		// nastupila je greška
					send(msgsock, buffer, 4, 0);
					break;
				}
